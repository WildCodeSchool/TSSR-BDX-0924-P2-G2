...
###### Script #########
while true; do
    echo -e "========================= ${cyan}Menu cible : Utilisateur${noColor} ========================="
    ...
    case $choice in
        1)
            echo "$date-$hour-$name-Menu cible_Utilisateur_Choix 1_Action sur la cible." >> $logPath
            source /home/wilder/TheScriptingProject/user/actionUser.sh
            ;;
        2)
            echo "$date-$hour-$name-Menu cible_Utilisateur_Choix 2_Information sur la cible." >> $logPath
            source /home/wilder/TheScriptingProject/user/infoUser.sh
            ;;
        3)
            echo "$date-$hour-$name-Menu cible_Utilisateur_Choix 3_Retour au menu de départ." >> $logPath
        ...
        4)
            ...
            ;;
        *)
            echo "$date-$hour-$name-Menu cible : Utilisateur_Réponse invalide, retour au menu actuel." >> $logPath
            echo -e "[${red}WARNING${noColor}] ${orange}Vous devez choisir l'une des options demandés ! ${noColor}"
            echo "Vous allez être redirigé au menu cible : Utilisateur."
            sleep 2
            clear
